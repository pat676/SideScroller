...
class DustBrown{
    ...
    static func runAction(node: SKEmitterNode){
        let removeAction = SKAction.customAction(withDuration: 0){_,_ in DustBrown.removeNode(node: node)}
    ...
    private static var _reusableNodes = ReusableNodes<SKEmitterNode>(label: "DustBrownNodeQueue")
    ...
    static func dequeReusableNode() -> SKEmitterNode?{
        ...
        if(node == nil || node!.parent != nil){
            node = SKEmitterNode(fileNamed: "DustBrown.sks")
        }
        node?.resetSimulation()
        node?.isPaused = false
        node?.alpha = 1
        if(node != nil){
            DustBrown.runAction(node: node!)
        }
        return node
    }
    ...
    static func preloadReusableNodes(amount: Int){
        for _ in 0..<amount{
            if let node = SKEmitterNode(fileNamed: "DustBrown.sks"){
                node.isPaused = true
                node.resetSimulation()
    ...
    static func removeNode(node: SKEmitterNode){
        DustBrown.queueReusableNode(node)
