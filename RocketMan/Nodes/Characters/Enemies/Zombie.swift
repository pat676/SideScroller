...
class Zombie: Enemy{
    ...
    convenience init(){
        self.init(imageNamed: "ZombieIdle")
        scale(to: ZOMBIE_SIZE)
        name = "Zombie"
        shouldDrawPhysicsFrame = true
    ...
    func addAnimations(){
        idleAction = SKNode.createAnimation(from: "ZombieIdle", timePerFrame: ZOMBIE_IDLE_TIME_PER_FRAME)
        walkAction = SKNode.createAnimation(from: "ZombieWalk", timePerFrame: ZOMBIE_MOVE_TIME_PER_FRAME)
    }
